...
from wxdata.gfs.paths import(
    ...
    gefs0p50_branch_path
)

sys.tracebacklimit = 0
logging.disable()

def process_gefs0p50_data(model,
                          cat,
                          members):

    
    paths = gefs0p50_branch_path(model, 
                                 cat,
                                 members)
    if cat == 'members':
...
def process_gefs0p50_secondary_parameters_data(model,
                          cat,
                          members):
...
def process_gefs0p25_data(model,
                          cat,
                          members):
